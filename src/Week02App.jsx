...
function ExpansionLab({ isKo }) {
  ...
  // stem plot scale (log)
  const W2 = 460, Hs = 260, pad2 = 42;
  const logMap = v => Math.max(Math.log10(Math.max(v, 1e-9)), -8);
  const P2 = usePlotScale(0.5, NC + 0.5, -8, 0.3, W2, Hs, pad2);
  ...
  return (
    <Card>
      ...
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
        ...
        <svg width={W2} height={Hs} viewBox={`0 0 ${W2} ${Hs}`} style={svgBox(W2)}>
          {cn.map((c, i) => {
            const n = i + 1, v = c * c;
          ...
          {[1, 5, 10, 15].map(n => (
            <text key={n} x={P2.X(n)} y={Hs - pad2 + 15} fill={C.textDim} fontSize={10} textAnchor="middle">{n}</text>
          ))}
          ...
          <text x={W2 - pad2} y={Hs - pad2 + 15} fill={C.textDim} fontSize={10} textAnchor="end">n</text>
        </svg>
      </div>
